...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):
        ...
        """
        # Add bias input (a constant 1) to the beginning of inActs
        bias_input = [1] + inActs

        return self.sigmoid(self.getWeightedSum(bias_input))
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        """YOUR CODE"""
        bias_input = [1] + inActs
        return self.sigmoidDeriv(self.getWeightedSum(bias_input))
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for i in range(len(self.weights)):
            each_modification = alpha * delta * bias_input[i]
            self.weights[i] += each_modification
            totalModification += abs(each_modification)
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        for each_layer in self.layers:
            layer_output = [every_perceptron.sigmoidActivation(activations[-1]) for every_perceptron in each_layer] #[[0.5, 0.6], [0.7, 0.8, 0.9], [0.75]], -1 index is the output
        ...
        for example in examples:#for each example
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in range(len(layer)):
                    ...
                    sumDeltaWeights = 0
                    for nextNeuronNum in range(len(nextLayer)):
                        sumDeltaWeights += nextLayer[nextNeuronNum].weights[neuronNum + 1] * deltas[0][nextNeuronNum]
                    # Calculate delta for this neuron
                    delta = gPrime * sumDeltaWeights
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    while iteration < maxItr:
        trainError, newWeight = nnet.backPropLearning(examplesTrain, alpha)
        if newWeight < weightChangeThreshold:
            break
        weightMod = newWeight
        iteration += 1

        if iteration % 10 == 0:
            print('Iteration %d; training error %f and weight change %f' % (iteration, trainError, weightMod))
    ...
    for input, target in examplesTest:
        output = nnet.feedForward(input)[-1]
        outputRound = []
        for num in output:
            roundedNum = round(num)
            outputRound.append(roundedNum)

        if outputRound == target:
            testCorrect += 1
        else:
            testError += 1
