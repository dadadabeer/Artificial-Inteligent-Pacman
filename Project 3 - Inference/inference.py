...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for position in all_positions:
            self.beliefs[position] = self.getObservationProb(observation, pacman_position, position, jail_p) * self.beliefs[position]
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            ...
            for newPos, p in newPosDist.items():
                # Updating belief for newPos = probability x old belief
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            # The weight for each particle = p(observation| Pacman's position, the particle's position, and the jail position)
            dd[particle] += self.getObservationProb(observation, pacman_pos, particle, jail_pos)

        # If total particles = zero weight, reinitialize uniformly
        if dd.total() == 0:
            self.initializeUniformly(gameState)
            return
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            # Sample a new position aand add it to new list of particles
