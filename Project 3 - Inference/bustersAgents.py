...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        self.ghostBeliefs[i - 1] for i in range(1, len(livingGhosts)) if livingGhosts[i]
    ]

        # Find the most likely position for each living ghost
        mostLikelyPositions = []
        for ghost_distance in livingGhostPositionDistributions:
            # Find the position with the highest belief for each ghost
            mostLikelyPosition = max(ghost_distance.items(), key=lambda item: item[1])[0]
        ...
        # action bringing pacman to closest ghost
        bestAction = None
        bestDistance = float('inf')
        for action in legalActions:
            successor = Actions.getSuccessor(pacmanPosition, action)
            distance = self.distancer.getDistance(successor, closestGhostPosition)
